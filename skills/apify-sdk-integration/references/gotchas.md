...
**Store search (`apify actors search`) paging is unreliable**
The underlying `/v2/store` API returns a `count` equal to the requested `limit`,
not the number of Actors actually in `items[]`, and it drops results from the
page. At `--limit 1` the `items[]` array comes back empty even when matches
exist; the deficit grows at higher `--offset`. Rules when searching:
- Always use `--limit 10` (or at least 3). Never `--limit 1`.
- Read `items[]` directly; ignore `count`.
- Treat an empty `items[]` as the only "no results" signal — and retry once with
  broader keywords before concluding no Actor exists.
